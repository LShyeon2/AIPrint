...
namespace BoxPrint.GUI.Views
{
    // 2024.07.15 RGJ
    /// <summary>
    /// StoredCarrierView.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class StoredCarrierView : System.Windows.Controls.Page
    {
        #region Variable
        #region Field
        private string strtag = string.Empty;
        private ViewModelStoredCarrierView vm;
        #endregion
        ...
        #region Methods
        #region Constructor
        public StoredCarrierView()
        {
            InitializeComponent();
            vm = new ViewModelStoredCarrierView();
            //DataContext = this;
            DataContext = vm;
        ...
        private void ButtonControl_Click(object sender, RoutedEventArgs e)
        {
            try //240404 RGJ 클릭 이벤트 예외 못잡으면 프로그램 다운 되므로 전부 예외처리함.
            {
                ...
                {
                    LogManager.WriteOperatorLog(string.Format("사용자가 {0} {1} 을/를 Click하였습니다.", "Stored CarrierView Log", senderBuffer.Tag.ToString()),
                        ...
                        "Stored Carrier", senderBuffer.Tag.ToString());
                }
            }
            catch (Exception ex)
            {
        ...
        private void _UIEventCollection_OnCultureChanged(string cultureKey)
        {
            ...
            vm.SearchCarrierCount = string.Format("{0} : {1}", TranslationManager.Instance.Translate("Carrier").ToString(), LogGrid.Items.Count);
