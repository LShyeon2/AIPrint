...
namespace Stockerfirmware.Modules.Conveyor
{
    ...
    /// 스택커 전에 버퍼 모듈
    /// </summary>
    public class CV_StackBufferModule : CV_BaseModule
    {
        public CV_StackBufferModule(string mName, bool simul) : base(mName, simul)
        {
            CVModuleType = eCVType.StackBuffer;
            SetReceiveStopDelay(1000);
        }
            ...
            while (!bStepEnd)
            {
                ...
                switch (LocalActionStep)
                {
                    case 0: //스톱퍼 업 동작
                        ...
                        break;
                    case 1: // 패드 다운 동작
                        CurrentActionDesc = "패드 다운 동작 중";
                        if (CVStackPadControl(false))//트레이 받을수 있도록 패드 내린다.
                        {
                            LocalActionStep++;
                        }
                        else
                        {
                            //패드 다운 동작실패 
                            errorResult = new CV_ActionResult(ModuleName, eCV_ActionResult.ErrorOccured, "Pad Down 동작 실패하였습니다.");
                            LocalActionStep = ErrorStep;
                        }
                        break;
                    case 2: //컨베이어 PIO 시작
                    ...
                    case 5: //배출 컨베이어 신호 OFF 확인
                        ...
                        break;
                    case 6:
                        CurrentActionDesc = "패드 업 동작 중";
                        if (CVStackPadControl(true))//트레이 패드를 올린다.
                        {
                            LocalActionStep++;
                        }
                        else
                        {
                            //패드 업 동작실패 
                            errorResult = new CV_ActionResult(ModuleName, eCV_ActionResult.ErrorOccured, "Pad Up 동작 실패하였습니다.");
                            LocalActionStep = ErrorStep;
                        }
                        break;
                    case 7:
                ...
                switch (LocalActionStep)
                {
                    ...
                    case 1: //진입 허가 대기
                        ...
                        if (NextCV.Internal_ToPrevCV_LoadRequest && !NextCV.Internal_ToPrevCV_Error)
                        {
                            ...
                            //여기서 패드 하강
                            if (!CVStackPadControl(false)) //패드 다운
                            {
                                //패드 다운 동작실패 
                                errorResult = new CV_ActionResult(ModuleName, eCV_ActionResult.ErrorOccured, "Pad Down 동작 실패하였습니다.");
                                LocalActionStep = ErrorStep;
                    ...
                    case 2: //Stop 신호 Off 면 배출되었다고 본다.
                        ...
                        if (!IN_STOP)
                        {
                            ...
                            //CV_RunStop();  //컨베이어 제어는 스택커모듈에서 하기때문에 여기서 안함.
        ...
        public override eCV_StopperState GetCVStopperState(bool ForwardStopper = true)
        {
            if (UseStopper)
            {
                ...
                if (ForwardStopper)
                {
                    ...
                    bool bDown = CCLinkManager.CCLCurrent.ReadIO(ModuleName, string.Format("CV_STOPPER_FWD_DOWN_CHECK"));
                    if (bUp && !bDown)
                    {
