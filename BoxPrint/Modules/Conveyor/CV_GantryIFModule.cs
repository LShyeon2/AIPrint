...
{
    public class CV_GantryIFModule : CV_BaseModule
    {
        public CV_GantryIFModule(string mName ,bool simul) : base(mName ,simul)
        {
            CVModuleType = eCVType.GantryIF;
        }
        ...
        protected override CV_ActionResult InitializeAction()
        {
            ...
            if (IsTrayExist())
            {
                ...
                else
                {
                    ...
                    //Gantry 인터락 상태 체크
                    if(!CheckGantryReleaseInterLock())
                    {
                        //신호 Off 돠어있으면 초기화 동작 중단
                        LogManager.WriteConsoleLog(eLogLevel.Info, "Module : {0}   트레이 초기화 이동 시도중지. 겐트리 인터락 이상입니다.", ModuleName);
                        return new CV_ActionResult(ModuleName, eCV_ActionResult.ErrorOccured, "트레이 초기화 이동 시도중지. 겐트리 인터락 이상입니다.");
                    }
            ...
            while (!bStepEnd)
            {
                ...
                switch (LocalActionStep)
                {
                    case 0: //스톱퍼 업 동작
                        ...
                        break;
                    case 1: //겐트리 PIO 시작
                        CurrentActionDesc = "장비(Gantry) 배출 요청 대기중";
                        ...
                        break;
                    case 2:  //배출 완료 시그널 대기
                        CurrentActionDesc = "장비(Gantry) 배출 완료 대기중";
                        ...
                        break; 
                    case 3: //Gantry 신호 OFF 확인
                        CurrentActionDesc = "장비(Gantry) 신호 OFF 대기중";
                        if (IsTimeOut(StepTimeOutDT, TrayTransTimeout)) // 스텝 타임아웃 체크
                        {
                            LogManager.WriteConsoleLog(eLogLevel.Info, "Module:{0} Step:{1} TrayLoadAction Time Out 발생", ModuleName, LocalActionStep); 
                            if (UseAutoRecovery)
                            ...
                            {
                                GlobalData.Current.Alarm_Manager.AlarmOccur("500", ModuleName); //EQ_INTERFACE_ERROR.                            
                                //GlobalData.Current.MainBooth.SetTowerLampSub(true); //SUB 경광등 ON 20210412. 이건 쓰는지 확인 필요
                                LocalActionStep = ErrorStep;
                            }
                            break;
                        }
            ...
            while (!bStepEnd)
            {
                ...
                switch (LocalActionStep)
                {
                    ...
                    case 0: //다음 CV모듈로  UnloadRequest On
                        ...
                        break;
                    case 1: //진입 허가 대기
                        CurrentActionDesc = "장비(EQ) 쪽 진입 동작 대기중 ";
                        if (IN_IF_READY && !IN_IF_COMPLETE)
                        {
                            if (UseStopper)
                            {
                        ...
                        break;
                    case 3: //Enty , Stop 신호 Off 면 배출되었다고 본다.
                        ...
                        if (!IN_STOP && !IN_ENTRY) //모든 재하감지 OFF 확인
                        {
                        ...
                        break;
                    case 4: //진입 완료 신호 대기
                        CurrentActionDesc = "장비(Gantry) 쪽 완료 신호 대기중";
                        if (IsTimeOut(StepTimeOutDT, TrayTransTimeout)) // 스텝 타임아웃 체크
                        {
                            LogManager.WriteConsoleLog(eLogLevel.Info, "Module:{0} Step:{1} TrayUnLoadAction Time Out 발생", ModuleName, LocalActionStep);
                            if (UseAutoRecovery)
                    ...
                    case 5: //수신 컨베이어 신호 Off 대기
                        CurrentActionDesc = "장비(Gantry) 쪽 신호 OFF 대기중";
                        if (IsTimeOut(StepTimeOutDT, TrayTransTimeout)) // 스텝 타임아웃 체크
                        {
                            LogManager.WriteConsoleLog(eLogLevel.Info, "Module:{0} Step:{1} TrayUnLoadAction Time Out 발생", ModuleName, LocalActionStep);
                            if (UseAutoRecovery)
        ...
        protected override CV_ActionResult ReceiveTrayAction()
        {
            ...
            while (!bStepEnd)
            {
                ...
                switch (LocalActionStep)
                {
                    case 0: //겐트리 인터락 해제 On 대기
                        CurrentActionDesc = "겐트리 인터락 해제 대기중 ";
                        if (CheckGantryReleaseInterLock())
                        {
                            LogManager.WriteConsoleLog(eLogLevel.Info, "Module:{0} Step:{1} SendTrayAction 겐트리 인터락 해제 확인", ModuleName, LocalActionStep);
                            LocalActionStep++;
                        }
                        break;
                    case 1: //스톱퍼 업 동작
                    ...
                    case 2: //컨베이어 PIO 시작
                        ...
                        if (PrevCV.Internal_ToNextCV_UnloadRequest && !PrevCV.Internal_ToNextCV_Error) //배출 요청 확인
                        {
                            ...
                            //if (!CheckGantryReleaseInterLock())
                            //{
                            //    LogManager.WriteConsoleLog(eLogLevel.Info, "Module:{0} Step:{1} RecieveTrayAction 겐트리 인터락 OFF 상태!", ModuleName, LocalActionStep);
                            //    errorResult = new CV_ActionResult(ModuleName, eCV_ActionResult.ErrorOccured, "컨베이어 Forward 동작전 겐트리 인터락 상태입니다..");
                            //    LocalActionStep = ErrorStep;
                            //    break;
                            //}
        ...
        protected override CV_ActionResult SendTrayAction()
        {
            ...
            while (!bStepEnd)
            {
                if (AutoManual != eCVAutoManual.Auto && LocalActionStep <= 2)
                {
                    CV_RunStop();
                    NextCVCommand = eCVCommand.SendTray;
                    return new CV_ActionResult(ModuleName, eCV_ActionResult.Complete, "Auto 상태가 해제되었습니다..");
                ...
                switch (LocalActionStep)
                {
                    case 0: //겐트리 인터락 해제 On 대기
                        ...
                        break;
                    case 1: //다음 CV모듈로  UnloadRequest On
                    ...
                    case 2: //진입 허가 대기
                        ...
                        if (NextCV.Internal_ToPrevCV_LoadRequest && !NextCV.Internal_ToPrevCV_Error)
                        {
                            ...
                            //if (!CheckGantryReleaseInterLock())
                            //{
                            //    LogManager.WriteConsoleLog(eLogLevel.Info, "Module:{0} Step:{1} SendTrayAction 겐트리 인터락 OFF 상태!", ModuleName, LocalActionStep);
                            //    errorResult = new CV_ActionResult(ModuleName, eCV_ActionResult.ErrorOccured, "컨베이어 Forward 동작전 겐트리 인터락 상태입니다..");
                            //    LocalActionStep = ErrorStep;
                            //    break;
                            //}
        ...
        //Gantry 가 유닛 포지션을 On 시켜야 컨베이어 동작가능
        public bool CheckGantryReleaseInterLock()
        {
            if (SimulMode)
            {
                return true;
            ...
            {
                bool UnitPosition = CCLinkManager.CCLCurrent.ReadIO(ModuleName, "CV_IN_IF_UNIT_POSITION");
                return UnitPosition;            
            }
        }
