...
namespace Stockerfirmware.Modules.Conveyor
{
    ...
    /// </summary>
    public class CV_TurnOHTIFModule : CV_BaseModule
    {
        ...
        public CV_TurnOHTIFModule(string mName, bool simul) : base(mName, simul)
        {
            CVModuleType = eCVType.TurnOHTIF;
        }
                ...
                switch (LocalActionStep)
                {
                    case 0: //Turn 동작시작
                        ...
                        if (CVStopperClose(true) && CVStopperClose(false)) //모든 스톱퍼를 닫는다.
                        {
                            ...
                            {
                                //턴 동작 실패
                                errorResult = new CV_ActionResult(ModuleName, eCV_ActionResult.ErrorOccured, "Turn 동작 실패하였습니다.");
                                LocalActionStep = ErrorStep;
                            }
                        }
                        else
        ...
        private void UnloadStep00_Start()
        {
            ...
            //2021.05.21 lim, 동작 전 Turn 상태로 대기 한다.
            if (GetCVTurnState() != eCV_TurnState.Turn)
            {
                if (!DoTurnAction())
                {
