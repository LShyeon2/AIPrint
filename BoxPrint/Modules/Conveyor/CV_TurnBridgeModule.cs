...
namespace Stockerfirmware.Modules.Conveyor
{
    ...
    //트레이 받고 -> 턴
    //트레이 배출하고 ->리턴
    public class CV_TurnBridgeModule : CV_BaseModule
    {
        public CV_TurnBridgeModule(string mName, bool simul) : base(mName, simul)
        {
            CVModuleType = eCVType.TurnBridge;
                ...
                switch (LocalActionStep)
                {
                    ...
                    case 3: //Stop 위치까지 트레이 전송
                        ...
                        if (IN_STOP) //스톱위치 확인
                        {
                            ...
                            Thread.Sleep(600);// 스톱 위치와 스톱퍼 위치 불일치로 대기시간 추가.
                            CV_RunStop();
                ...
                switch (LocalActionStep)
                {
                    ...
                    case 1: //진입 허가 대기
                        ...
                        if (NextCV.Internal_ToPrevCV_LoadRequest && !NextCV.Internal_ToPrevCV_Error)
                        {
                            ...
                            {
                                //2021.07.22 lim, 제품 방향을 위해 인, 아웃 방향이 다르다 
                                if (IsInPort)
                                {
                                    if (CVStopperOpen(true) && CVStopperClose(false)) //역방향 스톱퍼는 열고 정방향  스톱퍼는 닫는다.
                                    {
                                        //OK
                                    }
                                    else
                            ...
                            else //턴 이후에는 후진으로 보내야한다.
                            {
                                //허가 받고 컨베이어 가동
                                //2021.07.22 lim, 제품 방향을 위해 인, 아웃 방향이 다르다 
                                bool result = false; // CVBackwardRun(eCV_Speed.High);
                                if (IsInPort)  result = CVForwardRun(eCV_Speed.High);
                                else           result = CVBackwardRun(eCV_Speed.High);
                    ...
                    case 3: //Stop 신호 Off 면 배출되었다고 본다.
                        ...
                        else
                        {
                            ...
                            if (IsInPort)
                            {
                                if (!IN_STOP) //턴 이후에는 Entry 신호로 배출 판단
                                {
                                    NextCV.InsertTray(RemoveTray());// 트레이 정보를 옮겨준다.
                                    Thread.Sleep(TraySendRemainDelay);
                                    Internal_ToNextCV_OutComplete = true;
