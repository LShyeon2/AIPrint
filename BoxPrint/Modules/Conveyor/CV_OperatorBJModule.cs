...
{
    /// <summary>
    /// 작업자 인터페이스 하는 컨베이어 모듈 [범퍼자 전용]
    /// </summary>
    public class CV_OperatorBJModule : CV_BaseModule
    {
        public CV_OperatorBJModule(string mName, bool simul) : base(mName, simul)
        {
            ...
            while (!bStepEnd)
            {
                ...
                switch (LocalActionStep)
                {
                    ...
                    case 2: //RFID PIO Step 
                        //lsj SESS Bumpjar
                        bool DETECT1 = CCLinkManager.CCLCurrent.ReadIO(ModuleName, "CV_BUMPER_JAR_DETECT1");
                        bool DETECT2 = CCLinkManager.CCLCurrent.ReadIO(ModuleName, "CV_BUMPER_JAR_DETECT2");

                        if (!DETECT1 && !DETECT2)
                        {
                            LogManager.WriteConsoleLog(eLogLevel.Info, "CV_BUMPER_JAR_DETECT1 센서 확인");
                            return errorResult;
                        }
